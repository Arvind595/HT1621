 ...
 * The HT1621 is multi-function LCD driver with a 32x4 memory mapping capability. It has a simple
...
public:
    ...
    void inWrite( unsigned short segment, unsigned short number,char play, char pause);
	
	void inWrite( unsigned short segment, unsigned short number);
	
    void specialSeg(uint8_t binten,uint8_t binfiften);
	
	void specialSeg(String s,char en);
	
	void adWrite(unsigned short address, uint8_t value);
	
	// void aplWrite(unsigned short segment, char letter);
};
